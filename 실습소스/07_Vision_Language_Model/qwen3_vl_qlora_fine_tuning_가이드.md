...
10. [트러블슈팅](#10-트러블슈팅)
11. [관련 자료](#11-관련-자료)
12. [Google Colab에서 실행하기](#12-google-colab에서-실행하기)
...
## 12. Google Colab에서 실행하기

로컬(Windows + RTX 3060) 대신 **Google Colab**에서 동일 노트북을 실행하는 방법입니다.  
Colab은 세션이 끊기면 `/content` 디스크가 초기화되므로, **Google Drive 연동·산출물 백업**을 함께 설정하는 것을 권장합니다.

### 12.1 Colab vs 로컬 비교

| 항목 | 로컬 (3060 12GB) | Colab 무료 (T4 15GB) | Colab Pro (A100/L4 등) |
|------|------------------|----------------------|-------------------------|
| VRAM | 12GB | **~15GB** (merge·fp16 추론 여유↑) | 24~40GB+ |
| 학습 시간 | ~80분 | 비슷하거나 약간 빠름 | 더 빠름 |
| 세션 유지 | 무제한 | **최대 ~12시간**, 유휴 시 끊김 | 연장 가능 |
| 디스크 | 영구 | `/content` **임시** | 동일 (Drive 백업 필요) |
| bitsandbytes | Windows 별도 이슈 | **Linux — 호환 양호** | 양호 |

### 12.2 사전 준비

1. [Google Colab](https://colab.research.google.com/) 접속
2. Google 계정 로그인
3. (권장) [Google Drive](https://drive.google.com/) 여유 공간 **20GB+**
4. (필요 시) [Hugging Face](https://huggingface.co/) 계정 — gated 리소스용

### 12.3 노트북 올리기

**방법 A — 파일 업로드 (가장 단순)**

1. Colab 메뉴 **파일 → 노트 업로드**
2. `08_qwen3_vl_qlora_fine_tuning.ipynb` 선택

**방법 B — Drive에 두고 열기**

1. `.ipynb`를 Drive에 업로드
2. Drive에서 우클릭 → **연결 앱** → **Google Colaboratory**

**방법 C — GitHub 연동**

- 저장소에 노트북이 있으면 Colab에서 **파일 → 노트북 열기 → GitHub** 탭으로 URL 지정

### 12.4 GPU 런타임 설정

1. 메뉴 **런타임 → 런타임 유형 변경**
2. **하드웨어 가속기: GPU** 선택
3. (Pro) A100 / L4 등 고VRAM GPU 선택 가능

§1 import 셀 실행 후 GPU 확인:
...
- 무료: 보통 `Tesla T4` (~15GB)
- GPU 할당 실패 시: 잠시 후 재시도, 또는 Pro/다른 시간대 이용

### 12.5 패키지 설치 (Colab 전용)

Colab에는 `torch`가 기본 포함되어 있습니다. §1 **패키지 설치 셀**의 주석을 해제해 **맨 먼저** 실행하세요.

```python
%pip install -q "transformers>=4.57.0" "trl>=0.9.0" "peft>=0.10.0" "bitsandbytes>=0.43.0" "datasets>=2.18.0" accelerate pillow matplotlib requests
```

설치 후 **런타임 → 세션 다시 시작** → §1 import 셀부터 재실행을 권장합니다 (`%pip` 반영).

> `requirements.txt`의 `pip install -r`는 Colab에 파일이 없으면 실패합니다.  
> 업로드하지 않았다면 위 `%pip` 한 줄 설치로 충분합니다.

### 12.6 Hugging Face 로그인 (필요할 때만)

모델·데이터 다운로드가 거부되면 §1 import **앞**에 한 셀 추가:

```python
from huggingface_hub import notebook_login
notebook_login()  # 토큰 입력 (Read 권한)
```
...
### 12.7 Google Drive 연동 (강력 권장)

세션 종료 후에도 LoRA·체크포인트를 보존하려면 `OUTPUT_DIR`을 Drive로 지정합니다.

**Drive 마운트 셀** (§1 import 셀 **바로 위**에 추가):
...
**§1 하이퍼파라미터 셀**에서 `OUTPUT_DIR`만 변경:

```python
# Colab + Drive 예시
OUTPUT_DIR = "/content/drive/MyDrive/qwen3-vl-4b-qlora-sft-ko-1.5k"
```

- 학습·`save_model`·merge 결과가 Drive에 직접 저장됩니다.
- 세션이 끊겨도 **학습을 처음부터 다시 하지 않아도** 됩니다 (Drive에 `OUTPUT_DIR`이 남아 있는 경우).

**로컬 PC로 가져오기**

- Drive 웹에서 `qwen3-vl-4b-qlora-sft-ko-1.5k` 폴더 다운로드
- 또는 Colab에서 zip:

```python
!zip -r /content/drive/MyDrive/qwen3-lora.zip /content/drive/MyDrive/qwen3-vl-4b-qlora-sft-ko-1.5k
```

### 12.8 Colab 실행 순서 (요약)

로컬과 **동일한 섹션 순서**입니다. Colab에서만 추가·변경할 것:

| 단계 | 작업 |
|------|------|
| 0 | GPU 런타임 선택 |
| 1 | (선택) Drive 마운트 |
| 2 | `%pip install` 셀 실행 → (권장) 세션 재시작 |
| 3 | (선택) HF `notebook_login()` |
| 4 | §1 import — `OUTPUT_DIR`을 Drive 경로로 수정했는지 확인 |
| 5 | §2 → §3 → §4 → §5 → (§6) 순서대로 실행 |

상세 흐름은 [§3 전체 실행 흐름](#3-전체-실행-흐름)을 따릅니다.

### 12.9 Colab에서의 VRAM·섹션 팁

| 섹션 | Colab T4 (15GB) |
|------|-----------------|
| §3 fp16 베이스 추론 | ✅ 여유 (로컬 12GB보다 나음) |
| §4 QLoRA 학습 | ✅ |
| §5 adapter OFF/ON 비교 | ✅ (로컬과 동일 전략) |
| §6 merge + fp16 추론 | ⚠️ 가능하나 빡빡 — T4는 로컬보다 유리 |

**세션 끊김 대비**

- `trainer.train()` 전에 `OUTPUT_DIR`이 Drive인지 확인
- 학습 완료 직후 `trainer.save_model(OUTPUT_DIR)` 결과를 Drive에서 확인
- 장시간 학습 중: Colab 탭을 주기적으로 확인 (유휴 disconnect 방지)

### 12.10 Colab 세션 재시작 후 이어하기

로컬 [§8 부분 재실행 가이드](#8-부분-재실행-가이드)와 동일하며, Drive에 산출물이 있으면:

**학습 생략 — 비교만 (§5)**

```
Drive 마운트 → §1 (OUTPUT_DIR=Drive 경로) → §2 → §3 헬퍼 → §5 비교 블록
```

**학습 생략 — merge만 (§6)**

```
Drive 마운트 → §1 → §3 헬퍼 → §4 clear_memory 정의 → §6 merge
```

`OUTPUT_DIR`이 Drive에 있고 `adapter_model.safetensors`가 있으면 **80분 학습 재실행 불필요**.

### 12.11 Colab 자주 나는 오류

| 증상 | 해결 |
|------|------|
| `CUDA GPU가 필요합니다` | 런타임을 **GPU**로 변경 후 세션 재시작 |
| GPU 할당 안 됨 | 무료 한도 — 시간대 변경·Pro·다음날 재시도 |
| `No module named 'trl'` 등 | `%pip install` 후 **세션 재시작** |
| 학습 후 파일 사라짐 | `OUTPUT_DIR`을 `/content/drive/...`로 변경 |
| Drive 마운트 오류 | 마운트 셀 재실행, Drive 권한 허용 확인 |
| HF 401 / gated | `notebook_login()` 후 재시도 |
| 세션 timeout (~12h) | Drive에 checkpoint 저장 확인, `resume_from_checkpoint` 수동 추가 검토 |

### 12.12 Colab 최소 변경 체크리스트

로컬 노트북을 Colab에 **거의 그대로** 쓰려면 아래만 확인하면 됩니다.

- [ ] 런타임: **GPU**
- [ ] `%pip install` 셀 실행 (최초 1회)
- [ ] (권장) `drive.mount("/content/drive")`
- [ ] `OUTPUT_DIR = "/content/drive/MyDrive/qwen3-vl-4b-qlora-sft-ko-1.5k"`
- [ ] (필요 시) Hugging Face 로그인
- [ ] 이후 §1~§6 **로컬과 동일 순서** 실행

코드 로직·하이퍼파라미터·섹션 구조는 **수정 없이** 그대로 사용할 수 있습니다.

---

*문서 버전: `08_qwen3_vl_qlora_fine_tuning.ipynb` 기준 (mosshoon 데이터셋, adapter OFF/ON 비교, merge 후 GPU 비우기 추론, **Colab 실행 가이드** 반영)*
